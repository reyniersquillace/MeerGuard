...
class ReceiverBandCleaner(cleaners.BaseCleaner):
    ...
    def _set_config_params(self):
        ...
        self.configs.add_param('response', config_types.FloatPair,
                               aliases=['resp'],
                               nullable=True,
                               help='The range of frequencies between '
                                    'which the receiver has sensitivity. '
                                    'Any channels outside this range will '
                                    'be de-weighted.')
        self.configs.add_param('trimnum', config_types.IntVal,
                               help='The number of channels to de-weight '
                                    'at each edge of the band.')
        self.configs.add_param('trimfrac', config_types.FloatVal,
                               help='The fraction of each band-edge to '
                                    'de-weight (a floating-point number '
                                    'between 0 and 0.5).')
        ...
        self.configs.add_param('badsubints', config_types.IntOrIntPairList,
                               nullable=True,
                               help='Bad subints and/or (inclusive) '
                                    'subint-intervals to de-weight. Note: '
                                    'Subints are indexed starting at 0.')
        self.configs.add_param('badchans', config_types.IntOrIntPairList,
                               nullable=True,
                               help='Bad channels and/or (inclusive) '
                                    'channel-intervals to de-weight. Note: '
                                    'Channels are indexed starting at 0.')
        self.configs.add_param('badfreqs', config_types.FloatOrFloatPairList,
                               nullable=True,
                               help='Bad frequencies and/or (inclusive) '
